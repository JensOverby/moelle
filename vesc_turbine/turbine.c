...
static mutex_t bufprintf_mutex;
...
static char bufprintf_buffer[512];
static int bufprintf_index = 0;
static int bufprintf_index_lastpoll = 0;

void bufprintf(const char* format, ...) {
	chMtxLock(&bufprintf_mutex);

	va_list arg;
	va_start (arg, format);
	static char print_buffer[256];
	int len = vsnprintf(print_buffer, 255, format, arg);
	if (len > 255)
		len = 255;
	print_buffer[len] = '\0';
	len++;

	int index = (bufprintf_index + len) % 512;
	if (index < bufprintf_index) {
		int len_sub = 512-bufprintf_index;
 		memcpy((char*)((int)bufprintf_buffer+bufprintf_index), print_buffer, len_sub);
 		memcpy(bufprintf_buffer, print_buffer+len_sub, len-len_sub);
	}
	else {
 		memcpy(bufprintf_buffer+bufprintf_index, print_buffer, len);
	}
	va_end (arg);

	chMtxUnlock(&bufprintf_mutex);
}
...
void turbine_printf(const char* format, ...) {
	...
	len = vsnprintf(print_buffer + 1, 253, format, arg);
	if (len > 253)
		len = 253;
	print_buffer[len+1] = '\0';

	va_end (arg);

	if(len > 0) {
		comm_usb_send_packet((unsigned char*)print_buffer, len+2);
		//commands_send_packet((unsigned char*)print_buffer, len+2);
