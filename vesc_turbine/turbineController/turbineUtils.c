...
void printHelp()
{
  turbine_printf("Commands:");
  turbine_printf("-----------------");
  turbine_printf("help | prints this message");
  turbine_printf("ack | toggle self startup");
  turbine_printf("spinup | spinup turbine now");
  turbine_printf("initstartwind | sample startup wind");
  turbine_printf("windsense <1/0> | wind sensor on/off");
  turbine_printf("sample | sample Vin,Iout,Vout");
  turbine_printf("verbose <1/0> | verbose on/off");
  turbine_printf("duty <value> | set duty (0-255)");
  turbine_printf("dumpload <V-begin> <V-end> | use dump resistor (optional params)");
  turbine_printf("simrun <1/0> | simulate turbine run");
  turbine_printf("settings | print settings");
  turbine_printf("setsetting <id> <val> | set setting by id,value");
  turbine_printf("storesettings | store settings");
  turbine_printf("sensorparams <threshold> <gustminsecs> <gustmaxsecs> <gustcount> | set wind sensor parameters");
  turbine_printf("minruntime <sec> | set minimum ok run time");
  turbine_printf("commutate <cnt> | commutate motor forward");
  turbine_printf("poweroffset <value> | spinup pwm offset");
  turbine_printf("stop <offset> | break to offset voltage above Vout. Default 0");
  //turbine_printf);
}
...
float getExpectedPower(float rpm, float )
{
