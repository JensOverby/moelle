...
static systime_t tout;
static float tout_c;

void disableTimeout(int ms) {
	// Disable timeout
	tout = timeout_get_timeout_msec();
	tout_c = timeout_get_brake_current();
	timeout_reset();
	timeout_configure(ms, 0.0);
}
...
void turbineController_data_handler(unsigned char* data, unsigned int len)
{
	...
	//if (cmd > 0) {
	//	turbine_printf("CMD_HELP %d %d", cmd, len);
	//}
	cmd = CMD_ERROR;
}
...
{
  float windspeed = rpm * RPM_TO_TIP_SPEED_FACTOR / conf->tip_speed_ratio; // Unit is meter/sec
  //float windspeed = rpm * RPM_TO_WINDSPEED_PROPORTIONAL_FACTOR; // Unit is meter/sec
  float power = POWER_FACTOR * powf(windspeed, 3) - conf->W_losses;
  if (power < 0)
    return 0.;
  return power;
}
...
//					turbine_printf("Entering INIT state");
			...
			case SPINUP:
			{
				...
				/*for (float rpm=1; rpm<60; rpm++) {
					mcpwm_foc_set_openloop_duty(0.1f, rpm);
					chThdSleepMilliseconds(100);
				}*/

				spinUpSuccessFlag = false;
				mc_control_mode mode = CONTROL_MODE_NONE;
				//const float kaj = 2.f;
				//mc_interface_set_current(kaj);
				chThdSleepMilliseconds(5);
				for (int i=0; i<3000; i++) {
					mc_control_mode newmode = mcpwm_foc_get_control_mode();
					if (newmode != mode) {
						mode = newmode;
						//turbine_printf("mode %d", (int)mode);
					}
					if (tb_rpm > 20000) {
						spinUpSuccessFlag = true;
						break;
					}
					chThdSleepMilliseconds(5);
				}
				...
				turbine_printf("mode kaj");
				//turbine_printf("mode %d", (int)mode);

				if (spinUpSuccessFlag)
					turbine_printf("spinup success");
				else
					turbine_printf("spinup fail");

				//turbine_printf("rpm = %.2f", (double)rpmnow);
...
//				state = RUNNING_NO_CHARGE;
			...
			case RUNNING_NO_CHARGE:
			{
				...
				{
					if ((tb_converter_voltage_out > conf->V_out_min) && (tb_converter_voltage_out < conf->V_out_max))
					{
			...
			case CHARGING:
			{
				...
				if (tb_converter_voltage_out < conf->V_out_min) {
					turbine_printf("Vout_filter < VoutMin: %.2f < %.2f", (double)tb_converter_voltage_out, (double)conf->V_out_min);
					ok = false;
				}
		...
		#ifndef BENCH_TEST
				  if (tb_converter_voltage_out > conf->V_out_max || powerReal > conf->W_max) {
					if (!powerMaxReachedFlag) {
					  voltageTooHighTimeStampInSeconds = millis;
					  powerMaxReachedFlag = true;
					}
					else {
					  if (tb_converter_voltage_out > conf->V_out_max+0.5)
						setDumpload(true);
